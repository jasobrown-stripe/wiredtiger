  ...
  WT_GCC_FUNC_ATTRIBUTE((format(printf, 6, 7))) WT_GCC_FUNC_ATTRIBUTE((visibility("default")))
{
    ...
    va_start(ap, fmt);
    WT_IGNORE_RET(
      __eventv(session, false, FLD_ISSET(S2C(session)->json_output, WT_JSON_OUTPUT_ERROR), error,
  ...
  WT_GCC_FUNC_ATTRIBUTE((visibility("default")))
{
    ...
    va_start(ap, fmt);
    WT_IGNORE_RET(
      __eventv(session, false, FLD_ISSET(S2C(session)->json_output, WT_JSON_OUTPUT_ERROR), 0, func,
  ...
  const char *fmt, ...) WT_GCC_FUNC_ATTRIBUTE((format(printf, 4, 5))) WT_GCC_FUNC_ATTRIBUTE((cold))
{
    ...
    va_start(ap, fmt);
    WT_IGNORE_RET(
      __eventv(session, true, FLD_ISSET(S2C(session)->json_output, WT_JSON_OUTPUT_MESSAGE), 0, NULL,
        0, category, level, fmt, ap));
    va_end(ap);
}
